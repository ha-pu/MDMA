...
test_that("remove_toc_leaders preserves horizontal rules", {
  md <- "text\n\n---\n\nmore"
  expect_true(grepl("---", clean_markdown(md)))
})

# ── basic: soft hyphens ───────────────────────────────────────────────────────

test_that("repair_soft_hyphens joins hyphenated line breaks", {
  expect_equal(clean_markdown("algo-\nrithm"), "algorithm")
  expect_equal(clean_markdown("compre-\nhensive study"), "comprehensive study")
})

test_that("repair_soft_hyphens preserves mid-line hyphens", {
  expect_equal(clean_markdown("state-of-the-art"), "state-of-the-art")
})
...
test_that("repair_soft_hyphens only joins when next char is lowercase", {
  ...
  # is not also applied)
  md <- "word-\nUpper case follows"
  expect_true(grepl("word-\nUpper", clean_markdown(md, level = "basic"), fixed = TRUE))
})
...
test_that("join_wrapped_lines merges hard-wrapped paragraph lines", {
  md <- "This is a long\nline that wraps"
  expect_equal(clean_markdown(md, level = "moderate"), "This is a long line that wraps")
})

test_that("join_wrapped_lines does not merge across blank lines", {
  md <- "Paragraph one.\n\nParagraph two."
  result <- clean_markdown(md, level = "moderate")
  expect_true(grepl("\n\n", result))
})
...
test_that("join_wrapped_lines does not merge lines inside code blocks", {
  md <- "```\nline one\nline two\n```"
  expect_equal(clean_markdown(md, level = "moderate"), md)
})
...
test_that("deduplicate_running_lines removes repeated short lines", {
  ...
  md <- paste(c("Header", "", "Content 1.", "", "Header", "", "Content 2.", "", "Header"), collapse = "\n")
  result <- clean_markdown(md, level = "moderate")
...
test_that("aggressive applies all moderate and basic steps too", {
  md <- "42\n\nHeader\nlines\n\nHeader\nlines\n\nHeader\nlines\n\n© 2024 Corp"
  result <- clean_markdown(md, level = "aggressive")
...
test_that("invalid level throws an error", {
  expect_snapshot(clean_markdown("text", level = "extreme"), error = TRUE)
})

# ── vector input ─────────────────────────────────────────────────────────────

test_that("clean_markdown vectorises over a character vector", {
  input <- c("algo-\nrithm", "word\n\n42\n\nmore")
  result <- clean_markdown(input)
...
test_that("empty string returns empty string", {
  expect_equal(clean_markdown(""), "")
})

test_that("excess blank lines are collapsed", {
  md <- "a\n\n\n\nb"
  expect_equal(clean_markdown(md), "a\n\nb")
})

test_that("CRLF line endings are normalised", {
  md <- "line one\r\nline two"
  expect_equal(clean_markdown(md), "line one\nline two")
})
